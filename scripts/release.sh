...
- 复制时可以改名：单个条目时弹窗顶部出现「Copy as / 复制为」输入框，
  名字会参与重名校验（所以改个名字就能避开目标处的同名冲突）
- 复制多个条目时保持各自原名，界面会说明这一点

## 上一版（1.1.0）

- 右键菜单新增「复制到…」（快捷键 `⇧⌘C`）：在对话框里浏览库内目录并选择目标文件夹，
  复制走 **服务端**（`svnmucc cp`），不下载也不重新上传，原文件保持不动
- 目标非法时「复制到此处」按钮会禁用：目标就是源所在目录、把文件夹复制进它自己内部、
  目标已有同名项、目标目录加载失败（最后一条顺带修掉了一个真实缺陷）
- 支持复制文件夹及其整个子目录树
