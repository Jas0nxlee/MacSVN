...
![主界面](docs/preview-browser.png)

![上传确认](docs/preview-upload-confirm.png)

遇到同名文件夹等无法覆盖的情况会直接拦下：

![类型冲突](docs/preview-type-conflict.png)
...
| 账户验证 | 访问受保护的仓库时弹出登录框，可勾选"记住密码"（存入系统钥匙串）；证书不受信任时可勾选信任该服务器 |
...
| 新建文件夹 | `⇧⌘N` 在当前位置创建目录 |
| 右键菜单 | 打开 / 下载 / 重命名 / 删除 / 拷贝链接 / 新建文件夹 / 刷新 |
...
![缺少 svn 时的首屏引导](docs/preview-install-guide.png)
...
![安装 Subversion](docs/preview-install-progress.png)
...
2. 若仓库需要认证，会弹出登录框；勾选"记住密码"后下次自动登录。
...
- App 内部用系统钥匙串保存密码（仅在勾选"记住密码"时写入）。
...
脚本会自动建一个临时仓库、起一个带认证的 `svnserve`（随机端口，避免命中钥匙串缓存），然后跑完 19 项检查：核心自检 ×2（file:// 与 svn:// 认证）、拖入上传/重名覆盖/拖到子目录、库内拖动移动文件与目录、冲突拦截、重命名、删除、Homebrew 安装流程、登录成功与密码错误拒绝。结束后自动停掉服务并清理测试凭据。
